...
namespace CoreCodeCamp.Data
{
    public class CampRepository : ICampRepository
    {
        ...
        {
            _logger.LogInformation($"Removing an object of type {entity.GetType()} from context.");
        ...
        public async Task<Camp> GetCampAsync(string moniker, bool includeTalks)
        {
        ...
        {
            _logger.LogInformation($"Getting all Camps for date: {dateTime}");
            ...
            // filter by date and order it
            query = query.OrderByDescending(c => c.EventDate).Where(c => c.EventDate == dateTime);

            return await query.ToArrayAsync();
        }

        public async Task<Speaker[]> GetAllSpeakers()
        {
            _logger.LogInformation($"Getting Speakers");

            IQueryable<Speaker> query = _context.Speakers.OrderBy(s => s.LastName);
        ...
        public async Task<Speaker> GetSpeakerAsync(int speakerId)
        {
            ...
            IQueryable<Speaker> query = _context.Speakers.Where(s => s.SpeakerId == speakerId);
        ...
        {
            _logger.LogInformation($"Getting a Talks for a Camp");
